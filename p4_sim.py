# Original Authors: Henry Sampson, Ali Syeda
...
def simulate(InstructionBin,InstructionHex, output_file):
                ...
                while w < len(waySize):
                    ...
                    while j < len(wordSize):
                        if (cache[i][w][j] == Memory[imm + Register[int(fetch[6:11],2)] - 8192]):
                            ...
                            if p == 0:
                                print(str(Register[int(fetch[11:16], 2)]))
                                print(str(imm + Register[int(fetch[6:11],2)] - 8192))
                                print("Set:" + str(i) + " " + "Way:" + str(w) + " " + "Word:" + str(j))
                                print("Hit")
                                p = 1
                            cached = True
                        j = j + 1
    ...
    output_file.write("Total # of hits: " + str(hit)+ "\n")
    output_file.write("Total # of misses: " + str(miss)+ "\n")
    output_file.write("Total % of hit rate: " + str(hitRate) + "%"+ "\n")
